...
import es.udc.fi.ri.Practica1.Utils;
...
public class Reuters21578Parser {
	...
	public static List<String> handleDocument(String text) {
		...
		String  oldId = Utils.getIds("OLDID", text);	//extraer oldId
		String  newId = Utils.getIds("NEWID", text);	//extraer newId
	...
	private static String extract(String elt, String text, boolean allowEmpty) {
		...
		String startElt = "<" + elt + ">";
		String endElt = "</" + elt + ">";
		//encuentra la primera ocurrencia de starElt en text (empieza a contar desde 0)
		int startEltIndex = text.indexOf(startElt);	
		if (startEltIndex < 0) {	// si es menor que cero se supone que no esta
			if (allowEmpty)
				return "";
			throw new IllegalArgumentException("no start, elt=" + elt
					+ " text=" + text);
		}
		int start = startEltIndex + startElt.length();	// donde comienza a buscar
		int end = text.indexOf(endElt, start);			// posicion que concuerda
		if (end < 0)
			throw new IllegalArgumentException("no end, elt=" + elt + " text="
					+ text);
		return text.substring(start, end);	// string entre start y end
	}
